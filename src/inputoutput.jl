...
"""
struct LinearizationResult{T,X}
    A::T
    B::T
    C::T
    D::T
...
function linearize(sys::ODESystem; x0=nothing, u = unbound_inputs(sys), y = unbound_outputs(sys), numeric=false)
    ...
    linres = LinearizationResult(A, B, C, D, x, u, y)
    ...
    if numeric || x0 !== nothing
        ...
        linres = numeric_linearization(linres, sym2val)
    end

    # x = states(sysr, states(sysr)) # redo this to get the namespace right for later use
    # u = filter(isinput, x)
    # x = filter(!isinput, x)
    LinearizationResult(A, B, C, D, x, u, y)
end
...
"""
    numeric_linearization(linres, sym2val::Dict, eltype=Float64)
...
"""
function numeric_linearization(linres, sym2val::AbstractDict, ::Type{FT} = Float64) where FT
    function tofloat(x)
        xv = value(x)
    ...
    LinearizationResult(A, B, C, D, ntuple(i->getfield(linres, 4+i), fieldcount(typeof(linres))))
end
